...
def my_foodmin_page():
    # Get all data
    entered_restoename = flask.request.form.get('restoname')
    entered_fooditemid = flask.request.form.get('foodid')
    entered_restoloaction = flask.request.form.get('loc')
    entered_fooditemname = flask.request.form.get('itemname')
    entered_preperationcost = flask.request.form.get('precost')
    entered_sellingprice = flask.request.form.get('sellprice')
    entered_description = flask.request.form.get('descri')
    ...
    print("Create table ")
    my_query = '''CREATE TABLE foodmin_table(
        RESTO NAME    VARCHAR(100),
        FOOD ITEM ID INTEGER PRIMARY KEY AUTOINCREMENT,
        RESTO LOCATION   VARCHAR(100),
        FOOD ITEM NAME  VARCHAR(100),
        PREPERATION COST  INTEGER(100),
    ...
    my_query = f"INSERT INTO FOODMIN_TABLE(RESTAURANTNAME, RESTAURANTLOCATION, FOODITEMNAME, PREPERATIONCOST, SELLINGPRICE, DESCRIPTION,) VALUES('{entered_restoname}','{entered_restolocation}','{entered_foodname}', '{entered_preperationcost}', '{entered_sellingprice}', '{entered_description}')"
    my_db_cursor.execute(my_query)
    my_db_connection.commit()
    my_db_connection.close()
