...
@foodcourt_app.route('/vieworder', methods=['GET'])
def my_vieworder_page():
    ...
    my_db_cursor = my_db_connection.cursor()
    print("Done")
    print("check whether table exisist")
    my_db_cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='foodmin_table'")
    print("Done")
    ...
    if len(my_db_result) == 0:
        return "enter atleast one user to view"

    print("Executing select query")
    my_db_cursor.execute('SELECT * FROM foodmin_table')
    print("Done")
    ...
    return flask.render_template('vieworder.html', my_data=my_db_result)

# END POINT - 10 : http://127.0.0.1:5000/approved URL MAPPED to '/approved'
# --------------------
@foodcourt_app.route('/myorder/<int:fiid>', methods=['GET'])
def my_order_page(fiid):
        ...
        print("Executing select query")
        my_db_cursor.execute(f"UPDATE foodmin_table WHERE FOOD_ITEM_ID='{fiid}'")
        my_db_connection.commit()
        my_db_connection.close()
        print("Done")

        return flask.redirect('/vieworder')
...
def my_foodmenu_page():
    ...
    print("result=", my_db_result)
    # All the data is in my_db_result
    return flask.render_template('foodmenu.html', my_data=my_db_result)



# --------------------
# END POINT - 10 : http://127.0.0.1:5000/fooddetails URL MAPPED to '/fooddetails'
# --------------------
@foodcourt_app.route('/fooddetails', methods=['GET'])
def my_fooddetails_page():
    ...
    print("result=",my_db_result)
    # All the data is in my_db_result
    return flask.render_template('fooddetails.html', my_data=my_db_result)
